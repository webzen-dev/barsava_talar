  ...
  return (
    <div className="relative md:h-[60vh] overflow-hidden flex flex-col gap-3">
      <div className="flex justify-end items-center gap-4">
        <button
          onClick={() => prevSlider()}
          className="w-8 h-8 bg-[#dec39a] rounded-lg flex items-center justify-center cursor-pointer text-white text-lg"
        >
          <IoIosArrowForward />
        </button>
        <button
      ...
      <div ref={emblaRef} className="overflow-hidden flex-1 ">
        <div className="flex h-full">
          {slides.map((slide) => (
            <div
              key={slide.id}
              className="flex-[0_0_70%] md:flex-[0_0_50%] md:max-w-1/2 lg:flex-[0_0_calc(100%/4)] lg:max-w-1/3 px-2 h-full aspect-square"
            >
